...
RUN apk add --update --no-cache zlib-dev curl libpng-dev oniguruma-dev php7-gd composer npm pcre-dev php7-dev php7-sqlite3 php7-opcache php7-fileinfo \
    ...
    && chmod 0755 /usr/bin/caddy && docker-php-ext-configure gd && docker-php-ext-install gd opcache fileinfo && apk upgrade
