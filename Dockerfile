...
RUN apk add --update --no-cache zlib-dev curl libpng-dev oniguruma-dev php7-gd composer npm pcre-dev php7-dev \
    ...
    && chmod 0755 /usr/bin/caddy && docker-php-ext-configure opcache --enable-opcache && docker-php-ext-install gd sqlite3 #opcache
